 ...
 */
AsteroidSet::Asteroid::Asteroid(const Vec2 p, const Vec2 v) : Asteroid(p,v,3) {}
 ...
 */
AsteroidSet::Asteroid::Asteroid(const Vec2 p, const Vec2 v, int type) {
    position = p;
    velocity = v;
    setType(type);
}
 ...
 */
bool AsteroidSet::init(std::shared_ptr<JsonValue> data) {
    if (data) {
        // Reset all data
        current.clear();
        _pending.clear();
