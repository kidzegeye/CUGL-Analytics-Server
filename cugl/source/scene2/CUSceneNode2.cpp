...
bool SceneNode::initWithData(const AssetManager* manager,
    ...
    if (data->has("color")) {
        ...
        if (col->isString()) {
            ...
            _tintColor.r = std::max(std::min(col->get(0)->asInt(0),255),0);
            _tintColor.g = std::max(std::min(col->get(1)->asInt(0),255),0);
            _tintColor.b = std::max(std::min(col->get(2)->asInt(0),255),0);
            _tintColor.a = std::max(std::min(col->get(3)->asInt(0),255),0);
        }
    }
