...
bool Material::initWithInfo(const std::shared_ptr<MaterialInfo>& info, bool texture) {
    ...
    if (texture) {
        ...
        if (info->map_Kn != nullptr) {
            _normalMap = Texture::allocWithFile(info->map_Ks->path);
            _normalMap->setBindPoint(SPECULAR_BIND);
            if (_normalMap != nullptr) {
                _normalMap->setWrapS(info->map_Ks->wrapS);
                _normalMap->setWrapT(info->map_Ks->wrapT);
            }
        }
    }
    return true;
